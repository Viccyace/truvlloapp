...
const styles = `
  * { box-sizing:border-box; margin:0; padding:0; }
  ...
  .plan-trial-label { display:flex; justify-content:space-between; font-size:0.72rem; color:rgba(255,255,255,0.55); font-weight:600; margin-bottom:6px; gap:10px; flex-wrap:wrap; }
  ...
  .plan-upgrade-btn:hover:not(:disabled) { transform:translateY(-1px); box-shadow:0 8px 24px rgba(27,67,50,0.35); }
...
export default function SettingsPage() {
  ...
  const trialEndDate = trialDaysLeft
    ? new Date(
  ...
  const NOTIF_OPTIONS = [
    ...
    },
    {
      ...
      desc: "A heads-up 2 days before your free trial ends.",
    },
    {
      key: "weekly_report",
    ...
    },
    {
      ...
      desc: "Personalised saving tips from your AI coach, twice a week.",
    },
  ];
  ...
  const saveProfile = async () => {
    ...
    try {
      ...
      setToast(error?.message || "Could not update profile");
    } finally {
  ...
  const saveCurrency = async () => {
    ...
    try {
      ...
      setCurrSaved(true);
      setToast("Currency updated");
      setTimeout(() => setCurrSaved(false), 3000);
    } catch (error) {
      setToast(error?.message || "Could not update currency");
    } finally {
  ...
  return (
    <>
      ...
      {confirmModal && (
        <div
          className="modal-bg"
          onClick={() => !signingOut && setConfirmModal(null)}
        >
            ...
            <div className="avatar-section">
              <div className="avatar-circle">
                ...
                <div className="avatar-overlay">
                  <div className="avatar-overlay-text">Profile</div>
                </div>
              </div>
        ...
        <div className="settings-card" style={{ "--delay": "0.1s" }}>
          ...
          <div className="card-body">
            ...
            <SaveRow
              ...
              saved={currSaved}
              onSave={saveCurrency}
              onReset={() => setCurrency(profile?.currency || "NGN")}
            />
          </div>
        </div>
                ...
                <div className="plan-hero">
                  ...
                  <div className="plan-desc">
                    {profile?.subscription_ends_at
                      ? `Next billing ends ${new Date(profile.subscription_ends_at).toLocaleDateString("en-NG", { day: "numeric", month: "short", year: "numeric" })}`
