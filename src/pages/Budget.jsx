...
function CapModal({ onSave, onClose, usedCats = [], sym = "₦" }) {
  ...
  const submit = async () => {
    ...
    setLoading(true);
    try {
      await onSave({ category, limit: Number(limit) });
    } finally {
      setLoading(false);
    }
  };
            ...
            {available.map((c) => (
              <div
                ...
                style={{
                  padding: "8px 4px",
                  borderRadius: 10,
                  border: `1.5px solid ${category === c.id ? "var(--green-light)" : "var(--border)"}`,
                  background:
                    category === c.id ? "var(--green-pale)" : "var(--white)",
                  cursor: "pointer",
                  textAlign: "center",
                  transition: "all 0.18s",
                }}
    ...
    <>
      ...
      {modal === "new_cap" && (
        <CapModal
          ...
          onClose={() => setModal(null)}
          sym={sym}
          usedCats={caps.map((c) => normalizeCategory(c.cat))}
        />
      )}
                    ...
                    const usedPct =
                      Number(b.total_amount || b.amount || 0) > 0
                        ? Math.min(
                            100,
                            Math.round(
                              ((budgetSpentMap[b.id] || Number(b.spent || 0)) /
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className="budget-item-right">
                          ...
                          >
                            {sym}
                            {fmt(budgetSpentMap[b.id] || b.spent || 0)} spent
                          </div>
                        </div>
                        {isActive ? (
