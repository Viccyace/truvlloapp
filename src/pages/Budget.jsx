export default function BudgetPage() {
  ...
  const currentBudget =
    activeBudget || allBudgets.find((b) => b.is_active) || null;
  ...
  const updateCap = async (id, newLimit, category) => {
    try {
  ...
  const deleteCap = async (category) => {
    try {
      await deleteCategoryCap(category);
  ...
  const addCap = async () => {
    try {
      const usedCats = categoryCaps.map((c) =>
        normalizeCategory(c.category || c.cat),
      );
    ...
    <>
      ...
      {modal === "edit_budget" && currentBudget && (
        <BudgetModal
          budget={{
            ...currentBudget,
            start: currentBudget.start_date || "",
            end: currentBudget.end_date || "",
          }}
          onSave={saveBudget}
          ...
          <div className="hero-top">
            <div>
              ...
              <div className="hero-period">
                ...
                {currentBudget?.end_date ? `– ${currentBudget.end_date}` : ""} ·{" "}
                      ...
                      <div key={cap.id}>
                        ...
                        <CapRow
                          ...
                          onUpdate={(id, newLimit) =>
                            updateCap(id, newLimit, cap.category || cap.cat)
                          }
                          onDelete={() => deleteCap(cap.category || cap.cat)}
                        />
                      </div>
                    ))}
            ...
            <div className="section-card">
              <div className="section-card-header">
                <div>
                  ...
                  <div className="section-sub">
                    {allBudgets.length} budgets created
                  </div>
                </div>
              </div>
              <div className="section-card-body">
