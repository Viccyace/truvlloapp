      ...
      >
        ...
        <div>
          ...
          <div style={{ fontSize: "0.8rem", color: "#2D6A4F", marginTop: 2 }}>
            Connect WhatsApp in Settings to activate your AI agent.
          </div>
        </div>
      </div>
    );
...
export default function Dashboard() {
  const { displayName, profile, updateProfile, isPremiumOrTrial } = useAuth();
  ...
  const [analystInsight, setAnalystInsight] = useState(
    "Add some expenses and your AI spending breakdown will appear here automatically.",
  );
  const [coachTip, setCoachTip] = useState(
    "Add your first expense and your personalised savings tips will appear here.",
  );
  ...
  const refreshAI = useCallback(async () => {
    ...
    try {
      ...
      // Fire contextual upgrade for non-premium users with meaningful spend
      if (!isPremiumOrTrial && insightRes?.insight) {
        const topCat = expenses.reduce((acc, e) => {
          acc[e.category] = (acc[e.category] ?? 0) + Number(e.amount);
          return acc;
        }, {});
        const [cat, amt] =
          Object.entries(topCat).sort((a, b) => b[1] - a[1])[0] ?? [];
        if (amt > 10000)
          setUpgradeTrigger({ category: cat, amount: amt, sym: currSym });
      }
  ...
  return (
    <>
      {showPersonality && (
        <SpendingPersonalityCard onClose={() => setShowPersonality(false)} />
      )}
      {upgradeTrigger && (
        <ContextualUpgrade
