...
const styles = `
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .dash { display:flex; flex-direction:column; gap:24px; max-width:1200px; animation:fadeIn 0.35s ease; }

  .greeting { animation:fadeUp 0.4s ease; }
  ...
  @media(max-width:480px){ .summary-grid{ grid-template-columns:repeat(2,1fr); gap:12px; } }

  .sum-card {
    background:var(--white); border-radius:20px; padding:22px 20px;
    border:1.5px solid var(--border); transition:all 0.22s; cursor:default;
  ...
  .sum-icon { width:36px; height:36px; border-radius:10px; display:flex; align-items:center; justify-content:center; font-size:1rem; margin-bottom:14px; }
  ...
  .sum-label { font-size:0.72rem; font-weight:700; text-transform:uppercase; letter-spacing:0.08em; color:var(--ink-subtle); margin-bottom:6px; }
  .hero-card .sum-label { color:rgba(255,255,255,0.6); }
  .sum-value { font-family:'Playfair Display',serif; font-size:1.6rem; font-weight:900; color:var(--ink); line-height:1; }
  ...
  .sum-change { display:flex; align-items:center; gap:5px; margin-top:8px; font-size:0.75rem; font-weight:600; }
  ...
  .two-col { display:grid; grid-template-columns:1fr 380px; gap:20px; animation:fadeUp 0.4s ease 0.1s both; }
  @media(max-width:1100px){ .two-col{ grid-template-columns:1fr; } }

  .pace-card { background:var(--white); border-radius:20px; padding:28px; border:1.5px solid var(--border); }
  ...
  .status-pill { display:inline-flex; align-items:center; gap:5px; padding:4px 12px; border-radius:100px; font-size:0.72rem; font-weight:800; white-space:nowrap; }
  .pace-meta { display:grid; grid-template-columns:repeat(3,1fr); gap:12px; margin-bottom:20px; }
  .pace-meta-item { background:var(--bg); border-radius:12px; padding:12px 14px; }
  .pace-meta-label { font-size:0.68rem; font-weight:700; text-transform:uppercase; letter-spacing:0.07em; color:var(--ink-subtle); margin-bottom:4px; }
  .pace-meta-value { font-family:'Playfair Display',serif; font-size:1.1rem; font-weight:800; color:var(--ink); }
  ...
  .safe-amount { font-family:'Playfair Display',serif; font-size:2.6rem; font-weight:900; color:var(--white); line-height:1; margin-bottom:4px; }
  .safe-period { font-size:0.82rem; color:rgba(255,255,255,0.5); }
  .safe-footer { display:flex; justify-content:space-between; align-items:flex-end; }
  .safe-days   { font-size:0.8rem; color:rgba(255,255,255,0.5); }
  .safe-days strong { color:rgba(255,255,255,0.85); }
  .safe-ring   { width:52px; height:52px; position:relative; }
  ...
  .ai-card { background:var(--ink); border-radius:20px; padding:24px; border:1px solid rgba(255,255,255,0.07); position:relative; overflow:hidden; }
  ...
  .ai-tag { display:flex; align-items:center; gap:6px; margin-bottom:14px; }
  .ai-dot { width:6px; height:6px; border-radius:50%; animation:pulse 2s ease-in-out infinite; }
  ...
  .ai-card-footer { margin-top:16px; display:flex; align-items:center; justify-content:space-between; }
  .ai-refresh-btn { background:rgba(255,255,255,0.07); border:none; border-radius:8px; padding:6px 12px; font-size:0.75rem; font-weight:600; color:rgba(255,255,255,0.5); cursor:pointer; transition:all 0.18s; font-family:'Plus Jakarta Sans',sans-serif; }
  ...
  .nl-card { background:var(--white); border-radius:20px; padding:24px; border:1.5px solid var(--border); animation:fadeUp 0.4s ease 0.2s both; }
  .nl-header { display:flex; align-items:center; gap:10px; margin-bottom:16px; }
  .nl-icon  { width:36px; height:36px; border-radius:10px; background:var(--amber-pale); display:flex; align-items:center; justify-content:center; font-size:1rem; }
  ...
  .nl-input {
    ...
    color:var(--ink); background:var(--bg); outline:none; transition:all 0.2s;
  }
  ...
  .nl-parsed {
    margin-top:14px; background:var(--green-pale); border-radius:12px; padding:14px 16px;
    display:flex; align-items:center; justify-content:space-between; gap:12px;
    border:1px solid rgba(27,67,50,0.12); animation:scaleIn 0.3s ease;
  }
  ...
  .nl-confirm-btn { background:var(--green-light); color:var(--white); border:none; border-radius:8px; padding:7px 16px; font-family:'Plus Jakarta Sans',sans-serif; font-size:0.8rem; font-weight:700; cursor:pointer; white-space:nowrap; transition:all 0.2s; }
  .nl-confirm-btn:hover { background:var(--green-mid); }

  .quick-card { background:var(--white); border-radius:20px; padding:24px; border:1.5px solid var(--border); animation:fadeUp 0.4s ease 0.22s both; }
  ...
  .field-input {
    ...
    font-family:'Plus Jakarta Sans',sans-serif; font-size:16px; font-weight:500;
    color:var(--ink); background:var(--cream); outline:none; transition:all 0.2s;
  }
  ...
  .cat-pill { padding:6px 12px; border-radius:100px; border:1.5px solid var(--border); font-size:0.78rem; font-weight:600; cursor:pointer; transition:all 0.18s; color:var(--ink-muted); background:var(--white); display:flex; align-items:center; gap:5px; }
  ...
  .quick-footer { display:flex; gap:10px; margin-top:16px; }
  .quick-btn-ghost { flex:0; padding:11px 20px; border:1.5px solid var(--border); border-radius:12px; background:transparent; color:var(--ink-muted); font-family:'Plus Jakarta Sans',sans-serif; font-size:0.88rem; font-weight:600; cursor:pointer; transition:all 0.2s; }
  .quick-btn-ghost:hover { border-color:rgba(10,10,10,0.2); color:var(--ink); }
  .quick-btn-primary { flex:1; padding:12px; border-radius:12px; border:none; background:linear-gradient(135deg,var(--green-deep),var(--green-light)); color:var(--white); font-family:'Plus Jakarta Sans',sans-serif; font-size:0.95rem; font-weight:700; cursor:pointer; transition:all 0.22s; box-shadow:0 4px 16px rgba(27,67,50,0.25); display:flex; align-items:center; justify-content:center; gap:6px; }
  ...
  .recent-card { background:var(--white); border-radius:20px; padding:24px; border:1.5px solid var(--border); animation:fadeUp 0.4s ease 0.25s both; }
  .recent-header { display:flex; justify-content:space-between; align-items:center; margin-bottom:20px; }
  .see-all-btn { font-size:0.82rem; font-weight:700; color:var(--green-mid); background:none; border:none; cursor:pointer; transition:color 0.18s; }
  .see-all-btn:hover { color:var(--green-deep); }
  .expense-list { display:flex; flex-direction:column; gap:2px; }
  .expense-row { display:flex; align-items:center; gap:14px; padding:12px 10px; border-radius:12px; transition:background 0.18s; cursor:default; }
  .expense-row:hover { background:var(--bg); }
  .expense-cat-icon { width:40px; height:40px; border-radius:12px; display:flex; align-items:center; justify-content:center; font-size:1.05rem; flex-shrink:0; }
  .expense-info { flex:1; min-width:0; }
  .expense-desc { font-size:0.9rem; font-weight:600; color:var(--ink); white-space:nowrap; overflow:hidden; text-overflow:ellipsis; }
  .expense-meta { font-size:0.75rem; color:var(--ink-subtle); margin-top:2px; }
  .expense-amount { font-family:'Playfair Display',serif; font-size:1rem; font-weight:800; color:var(--ink); flex-shrink:0; }
  .expense-amount.large { color:var(--red); }
  .expense-row-actions { display:none; gap:6px; }
  .expense-row:hover .expense-row-actions { display:flex; }
  .exp-action-btn { width:28px; height:28px; border-radius:7px; border:none; background:var(--cream-dark); color:var(--ink-subtle); font-size:0.75rem; cursor:pointer; display:flex; align-items:center; justify-content:center; transition:all 0.18s; }
  .exp-action-btn:hover { background:var(--border); color:var(--ink); }
  .exp-action-btn.del:hover { background:var(--red-pale); color:var(--red); }
  .empty-state { text-align:center; padding:40px 20px; color:var(--ink-subtle); }
  .empty-icon  { font-size:2.5rem; margin-bottom:12px; }
  .empty-title { font-family:'Playfair Display',serif; font-size:1.1rem; font-weight:700; color:var(--ink); margin-bottom:6px; }
  .empty-sub   { font-size:0.85rem; line-height:1.6; }

  .toast { position:fixed; bottom:24px; right:24px; z-index:200; background:var(--ink); color:var(--white); padding:14px 20px; border-radius:14px; font-size:0.875rem; font-weight:600; display:flex; align-items:center; gap:10px; box-shadow:0 8px 32px rgba(0,0,0,0.25); animation:scaleIn 0.3s ease; }
...
const AI_TIPS = [
  ...
  "Your Netflix and Showmax subscriptions overlap by 5 days each month. Cancelling one saves <strong>₦4,800/month</strong>.",
];

// Stable — defined outside component so it never changes between renders
const NL_EXAMPLES = [
  "spent 4500 on lunch",
  "bolt ride 2800",
  ...
  return (
    <div className="pace-card">
      ...
      <p className="pace-bar-caption">
        ...
        {spent <= expected
          ? " You're ahead of pace — great discipline this month."
...
function NLEntry({ onAdd }) {
  ...
  }, []); // no deps needed — NL_EXAMPLES is module-level constant, idxRef is stable
...
// Toast: onDone is stable (always () => setToast(null)) so safe to use directly
function Toast({ msg, onDone }) {
  useEffect(() => {
    const t = setTimeout(onDone, 2800);
    return () => clearTimeout(t);
  }, [onDone]); // onDone is stable — defined inline but parent never changes it
