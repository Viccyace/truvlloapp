...
const styles = `
  * { box-sizing:border-box; margin:0; padding:0; }
  ...
  .premium-badge { position:absolute; top:-6px; right:-6px; background:var(--amber); color:var(--ink); font-size:0.55rem; font-weight:800; padding:2px 6px; border-radius:100px; text-transform:uppercase; letter-spacing:0.06em; }
  ...
  .stat-val { font-family:'Playfair Display',serif; font-size:1.4rem; font-weight:900; color:var(--ink); }
  ...
  .search-icon { color:var(--ink-subtle); font-size:0.9rem; flex-shrink:0; }
  .search-input {
  ...
  .search-clear { background:none; border:none; cursor:pointer; color:var(--ink-subtle); font-size:0.9rem; padding:2px; transition:color 0.18s; }
  ...
  .view-btn { width:34px; height:34px; border:none; border-radius:8px; background:transparent; cursor:pointer; font-size:0.9rem; transition:all 0.18s; color:var(--ink-subtle); display:flex; align-items:center; justify-content:center; }
  ...
  .exp-cat-icon { width:44px; height:44px; border-radius:13px; display:flex; align-items:center; justify-content:center; font-size:1.1rem; flex-shrink:0; }
  ...
  .exp-act-btn { width:28px; height:28px; border-radius:8px; border:none; background:var(--cream-dark); color:var(--ink-subtle); font-size:0.75rem; cursor:pointer; display:flex; align-items:center; justify-content:center; transition:all 0.18s; }
  ...
  .td-icon { width:36px; height:36px; border-radius:10px; display:flex; align-items:center; justify-content:center; font-size:0.95rem; }
  ...
  .modal-close { width:32px; height:32px; border-radius:8px; border:none; background:var(--cream-dark); color:var(--ink-subtle); cursor:pointer; font-size:0.9rem; display:flex; align-items:center; justify-content:center; transition:all 0.18s; }
  ...
  .cat-select-item { padding:8px 4px; border-radius:10px; border:1.5px solid var(--border); cursor:pointer; text-align:center; transition:all 0.18s; }
  ...
  .cat-select-icon { font-size:1.1rem; }
  .cat-select-label { font-size:0.62rem; font-weight:700; color:var(--ink-muted); margin-top:3px; }
  ...
  .modal-delete { flex:0; padding:12px 16px; border-radius:12px; border:1.5px solid var(--red-pale); background:var(--red-pale); color:var(--red); font-family:'Plus Jakarta Sans',sans-serif; font-size:0.875rem; font-weight:700; cursor:pointer; transition:all 0.2s; }
  ...
  .rec-cat-icon { width:38px; height:38px; border-radius:11px; display:flex; align-items:center; justify-content:center; font-size:1rem; }
  ...
  .premium-gate {
    ...
    background:var(--ink); border-radius:14px; padding:14px 16px;
    display:flex; gap:10px; align-items:flex-start;
    box-shadow:0 8px 32px rgba(0,0,0,0.2); max-width:280px; z-index:10;
    animation:scaleIn 0.3s ease;
  }
  .gate-icon { font-size:1.2rem; flex-shrink:0; }
...
// ── Data ──────────────────────────────────────────────────────────────────────
const CATEGORIES = [
  { id:"food",      icon:"🍔", label:"Food",       bg:"#FFF3E0", color:"#E65100" },
  { id:"transport", icon:"🚗", label:"Transport",  bg:"#E8F5E9", color:"#2E7D32" },
  { id:"bills",     icon:"🏠", label:"Bills",      bg:"#FCE4EC", color:"#C2185B" },
  { id:"shopping",  icon:"🛍️", label:"Shopping",   bg:"#F3E5F5", color:"#7B1FA2" },
  { id:"health",    icon:"💊", label:"Health",     bg:"#E0F7FA", color:"#00838F" },
  { id:"airtime",   icon:"📱", label:"Airtime",    bg:"#E3F2FD", color:"#1565C0" },
  { id:"entertain", icon:"🎬", label:"Entertain.", bg:"#F9FBE7", color:"#558B2F" },
  { id:"other",     icon:"💼", label:"Other",      bg:"#F5F5F5", color:"#616161" },
];

const CAT_MAP = Object.fromEntries(CATEGORIES.map(c => [c.id, c]));

const MOCK_EXPENSES = [
  { id:1,  desc:"Chicken Republic — Lunch",    cat:"food",      amount:4500,  date:"2026-03-16", note:"" },
  { id:2,  desc:"Bolt — Office commute",       cat:"transport", amount:2800,  date:"2026-03-16", note:"Morning trip" },
  { id:3,  desc:"Netflix subscription",        cat:"entertain", amount:4800,  date:"2026-03-15", note:"" },
  { id:4,  desc:"Shoprite groceries",          cat:"food",      amount:18500, date:"2026-03-14", note:"Weekly shopping" },
  { id:5,  desc:"MTN Data bundle — 10GB",      cat:"airtime",   amount:3000,  date:"2026-03-13", note:"" },
  { id:6,  desc:"Chicken Republic — Dinner",   cat:"food",      amount:5200,  date:"2026-03-12", note:"" },
  { id:7,  desc:"EKEDC electricity bill",      cat:"bills",     amount:15000, date:"2026-03-10", note:"March prepaid" },
  { id:8,  desc:"Uber — Airport pickup",       cat:"transport", amount:8500,  date:"2026-03-09", note:"Client visit" },
  { id:9,  desc:"Zara — Work trousers",        cat:"shopping",  amount:22000, date:"2026-03-08", note:"" },
  { id:10, desc:"Pharmacy — Cold medicine",    cat:"health",    amount:3500,  date:"2026-03-07", note:"" },
  { id:11, desc:"Coldstone — Date night",      cat:"entertain", amount:6500,  date:"2026-03-06", note:"" },
  { id:12, desc:"Bolt — Supermarket trip",     cat:"transport", amount:1900,  date:"2026-03-05", note:"" },
  { id:13, desc:"Mr Biggs — Breakfast",        cat:"food",      amount:2100,  date:"2026-03-04", note:"" },
  { id:14, desc:"Airtel recharge",             cat:"airtime",   amount:1500,  date:"2026-03-03", note:"" },
  { id:15, desc:"DSTV subscription",           cat:"entertain", amount:9500,  date:"2026-03-02", note:"Monthly" },
];

const MOCK_RECURRING = [
  { id:1, desc:"Netflix",          cat:"entertain", amount:4800,  freq:"Monthly",  next:"Apr 1" },
  { id:2, desc:"DSTV Compact",     cat:"entertain", amount:9500,  freq:"Monthly",  next:"Apr 2" },
  { id:3, desc:"Gym membership",   cat:"health",    amount:15000, freq:"Monthly",  next:"Apr 1" },
  { id:4, desc:"iCloud 50GB",      cat:"airtime",   amount:900,   freq:"Monthly",  next:"Apr 5" },
];
...
function ExpenseModal({ expense, onSave, onDelete, onClose }) {
  ...
  const submit = () => {
    ...
    setLoading(true);
    setTimeout(() => {
      onSave({ id: expense?.id ?? Date.now(), desc, amount: Number(amount), cat, date, note });
      setLoading(false);
    }, 600);
  };
      ...
      <div className="modal" onClick={e => e.stopPropagation()}>
        ...
        <div className="modal-grid">
          <div className="field-wrap full">
            ...
            <input className={`field-input${errors.desc ? " error" : ""}`} type="text"
              placeholder="e.g. Chicken Republic lunch"
              value={desc} onChange={e => { setDesc(e.target.value); setErrors(x => ({...x,desc:""})); }} />
            {errors.desc && <span className="field-error">{errors.desc}</span>}
          </div>
          <div className="field-wrap">
            <label className="field-label">Amount</label>
            <div className={`amount-prefix-wrap${errors.amount ? " error" : ""}`} style={errors.amount ? {borderColor:"var(--red)"} : {}}>
              <span className="amount-prefix">₦</span>
              <input className="field-input" type="text" inputMode="numeric" placeholder="0"
                value={amount} onChange={e => { setAmount(e.target.value.replace(/[^0-9]/g,"")); setErrors(x => ({...x,amount:""})); }} />
            </div>
          ...
          <div className="field-wrap full">
            ...
            <div className="cat-select-grid">
              {CATEGORIES.map(c => (
                <div key={c.id} className={`cat-select-item${cat === c.id ? " active" : ""}`} onClick={() => setCat(c.id)}>
                  <div className="cat-select-icon">{c.icon}</div>
          ...
          <div className="field-wrap full">
            ...
            <input className="field-input" type="text" placeholder="Any extra detail..."
              value={note} onChange={e => setNote(e.target.value)} />
          </div>
        </div>
        <div className="modal-footer">
          {isEdit && <button className="modal-delete" onClick={() => onDelete(expense.id)}>🗑</button>}
          <button className="modal-cancel" onClick={onClose}>Cancel</button>
          <button className="modal-submit" onClick={submit} disabled={loading}>
            {loading ? <div className="spinner" /> : (isEdit ? "Save changes" : "Add expense")}
          </button>
        </div>
      </div>
    </div>
...
export default function ExpensesPage() {
  ...
  const isPremium = false; // swap with useAuth().isPremiumOrTrial
  ...
  const exportCSV = () => {
    ...
    const rows = [
      ...
      ...expenses.map(e => [e.date, `"${e.desc}"`, CAT_MAP[e.cat]?.label ?? e.cat, e.amount, `"${e.note ?? ""}"`])
    ];
    const csv  = rows.map(r => r.join(",")).join("\n");
    const blob = new Blob([csv], { type:"text/csv" });
    const url  = URL.createObjectURL(blob);
    const a    = document.createElement("a"); a.href=url; a.download="truvllo-expenses.csv"; a.click();
      ...
      <div className="page">
        ...
        <div className="page-header">
          <div>
            ...
            <div className="page-sub">{expenses.length} transactions · March 2026</div>
          </div>
        ...
        <div className="stats-row">
          ...
          <div className="stat-card">
            ...
            <div className="stat-val green">{topCat?.icon} {topCat?.label ?? "—"}</div>
        ...
        <div className="toolbar">
          <div className="search-wrap" onClick={() => searchRef.current?.focus()}>
            ...
            <input ref={searchRef} className="search-input" placeholder="Search expenses..."
              value={search} onChange={e => { setSearch(e.target.value); setPage(1); }} />
            {search && <button className="search-clear" onClick={() => setSearch("")}>✕</button>}
          </div>

          <div className="filter-group">
            <div className={`filter-pill${catFilter === "all" ? " active" : ""}`} onClick={() => { setCatFilter("all"); setPage(1); }}>
              All
            </div>
            {CATEGORIES.map(c => (
              <div key={c.id} className={`filter-pill${catFilter === c.id ? " active" : ""}`}
                onClick={() => { setCatFilter(catFilter === c.id ? "all" : c.id); setPage(1); }}>
                {c.icon} {c.label}
              </div>
            ))}
          </div>
        ...
        {paginated.length === 0 ? (
          <div className="empty">
            ...
            <p className="empty-sub">{search ? `No results for "${search}"` : "Try a different filter or add a new expense."}</p>
          </div>
        ) : view === "card" ? (
            ...
            {paginated.map(e => {
              ...
              return (
                <div key={e.id} className="expense-card">
                  <div className="exp-cat-icon" style={{ background: c.bg }}>{c.icon}</div>
                  <div className="exp-body">
                    <div className="exp-desc">{e.desc}</div>
                    <div className="exp-meta">
                      <span className="exp-cat-pill" style={{ background: c.bg, color: c.color }}>{c.label}</span>
                  ...
                  <div className="exp-right">
                    ...
                    <div className="exp-actions">
                      <button className="exp-act-btn" onClick={() => setModal(e)} title="Edit">✎</button>
                      <button className="exp-act-btn del" onClick={() => deleteExpense(e.id)} title="Delete">✕</button>
                    </div>
                  </div>
                </div>
              );
          ...
          <div className="table-wrap">
            <table>
              ...
              <tbody>
                {paginated.map(e => {
                  ...
                  return (
                    <tr key={e.id}>
                      <td><div className="td-icon" style={{ background: c.bg }}>{c.icon}</div></td>
                      <td><div className="td-desc">{e.desc}</div>{e.note && <div style={{fontSize:"0.72rem",color:"var(--ink-subtle)",marginTop:2}}>{e.note}</div>}</td>
                      <td><span className="exp-cat-pill" style={{ background:c.bg, color:c.color, padding:"3px 10px", borderRadius:"100px", fontSize:"0.72rem", fontWeight:700 }}>{c.label}</span></td>
                      <td style={{color:"var(--ink-subtle)",fontSize:"0.82rem"}}>{fmtDate(e.date)}</td>
                      <td className="right"><div className="td-amount">−₦{fmt(e.amount)}</div></td>
                      <td className="right">
                        <div className="td-actions">
                          <button className="exp-act-btn" onClick={() => setModal(e)}>✎</button>
                          <button className="exp-act-btn del" onClick={() => deleteExpense(e.id)}>✕</button>
                        </div>
                      </td>
                    </tr>
                  );
            ...
            {totalPages > 1 && (
              <div className="pagination">
                <div className="page-info">Showing {(page-1)*PAGE_SIZE+1}–{Math.min(page*PAGE_SIZE, filtered.length)} of {filtered.length}</div>
        ...
        <div className="recurring-section" style={{ position:"relative" }}>
          <div className="section-header">
            <div>
              ...
              <div className="section-sub">Fixed costs that repeat every month</div>
            </div>
            {!isPremium && (
              <span style={{ fontSize:"0.72rem", background:"var(--amber-pale)", color:"var(--amber)", padding:"4px 12px", borderRadius:"100px", fontWeight:800, border:"1px solid rgba(212,160,23,0.2)" }}>
                ✦ Premium
              </span>
            )}
          </div>
          <div className="recurring-grid" style={{ opacity: isPremium ? 1 : 0.55, filter: isPremium ? "none" : "blur(1px)", pointerEvents: isPremium ? "auto" : "none" }}>
            {recurring.map(r => {
              const c = CAT_MAP[r.cat] ?? CAT_MAP.other;
              return (
                <div key={r.id} className="rec-card">
                  <div className="rec-card-top">
                    <div className="rec-cat-icon" style={{ background: c.bg }}>{c.icon}</div>
                    <div className="rec-actions">
                      <button className="exp-act-btn del" onClick={() => deleteRecurring(r.id)}>✕</button>
                    </div>
                  </div>
                  <div className="rec-desc">{r.desc}</div>
                  <div className="rec-meta">
                    <span className="rec-freq">{r.freq}</span>
            ...
            <div className="rec-add-card">+ Add recurring</div>
          </div>
          ...
          {(!isPremium || showGate) && (
            <div className="premium-gate">
              ...
              <div className="gate-text">
                ...
                Recurring expenses, CSV export, and category caps are available on the Premium plan.
                <div><button className="gate-btn">Upgrade — ₦6,500/mo</button></div>
              </div>
            </div>
          )}
        </div>
