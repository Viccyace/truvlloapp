...
const styles = `
  * { box-sizing:border-box; margin:0; padding:0; }
  ...
  /* ── SAVE ROW ────────────────────── */
  .save-row { display:flex; align-items:center; justify-content:flex-end; gap:12px; margin-top:20px; }
  ...
  .plan-trial-label { display:flex; justify-content:space-between; font-size:0.72rem; color:rgba(255,255,255,0.55); font-weight:600; margin-bottom:6px; }
...
const PLAN_FEATURES_FREE    = ["Unlimited budgets","Expense logging","Budget pace","Safe-to-spend"];
const PLAN_FEATURES_PREMIUM = ["All AI features","Category caps","Recurring expenses","Advanced charts","CSV export","Habit streaks"];
...
const STRENGTH_COLORS = ["var(--border)","var(--red)","var(--amber)","var(--amber-light)","var(--green-light)"];
...
export default function SettingsPage() {
  ...
  // Computed once — trialDaysLeft is a constant (swap with useAuth() later)
  ...
  const saveProfile = () => {
    ...
    setTimeout(() => { setProfileLoading(false); setProfileSaved(true); setTimeout(() => setProfileSaved(false), 3000); }, 900);
  };
  ...
  const savePassword = () => {
    ...
    if (!newPw || newPw.length < 8) e.new = "Password must be at least 8 characters";
    if (newPw !== confirmPw) e.con = "Passwords don't match";
    if (Object.keys(e).length) { setPwErrors(e); return; }
    setPwErrors({}); setPwLoading(true);
    setTimeout(() => { setPwLoading(false); setPwSaved(true); setCurrentPw(""); setNewPw(""); setConfirmPw(""); setTimeout(() => setPwSaved(false), 3000); }, 900);
  };
  ...
  const saveCurrency = () => {
    setCurrLoading(true);
    setTimeout(() => { setCurrLoading(false); setCurrSaved(true); setTimeout(() => setCurrSaved(false), 3000); }, 700);
  };
  ...
  const saveNotifs = () => {
    ...
    setTimeout(() => { setNotifLoading(false); setNotifSaved(true); setTimeout(() => setNotifSaved(false), 3000); }, 700);
  };

  const NOTIF_OPTIONS = [
    { key:"daily_summary",   title:"Daily spending summary",     desc:"A quick recap of your spending sent every evening." },
    { key:"over_pace_alert", title:"Over-pace alerts",           desc:"Get notified immediately when your spending exceeds expected pace." },
    { key:"trial_reminder",  title:"Trial expiry reminder",      desc:"A heads-up 2 days before your free trial ends." },
    { key:"weekly_report",   title:"Weekly budget report",       desc:"Full spending breakdown delivered every Monday morning." },
    { key:"tips_and_advice", title:"AI tips & savings advice",   desc:"Personalised saving tips from your AI coach, twice a week." },
  ];

  // ── Danger zone ───────────────────────────────────────────────────────────────
  const [confirmModal, setConfirmModal] = useState(null); // null | "logout" | "delete"
  const [toast, setToast] = useState(null);

  const handleConfirm = () => {
    if (confirmModal === "logout")  setToast("Signed out successfully");
    if (confirmModal === "delete")  setToast("Account deletion request submitted");
    setConfirmModal(null);
  };

  const initials = `${firstName[0] ?? ""}${lastName[0] ?? ""}`.toUpperCase();
  const planLabel = isTrialing ? "Premium Trial" : isPremium ? "Premium" : "Free";
    ...
    <>
      ...
      {confirmModal && (
        <div className="modal-bg" onClick={() => setConfirmModal(null)}>
          <div className="confirm-modal" onClick={e => e.stopPropagation()}>
            <div className="confirm-icon">{confirmModal === "logout" ? "👋" : "⚠️"}</div>
            <div className="confirm-title">{confirmModal === "logout" ? "Sign out?" : "Delete account?"}</div>
      ...
      <div className="page">
        ...
        <div className="page-header">
          ...
          <div className="page-sub">Manage your account, preferences, and plan</div>
        </div>
        ...
        <div className="settings-card" style={{ "--delay": "0.04s" }}>
          <div className="card-header">
            <div>
              ...
              <div className="card-sub">Your name, email, and contact details</div>
            </div>
          </div>
          <div className="card-body">
            <div className="avatar-section">
              <div className="avatar-circle">
                ...
                <div className="avatar-overlay">
                  <div className="avatar-overlay-text">Change<br />photo</div>
                </div>
              </div>
            ...
            <SaveRow loading={profileLoading} saved={profileSaved} onSave={saveProfile}
              onReset={() => { setFirstName("Adaeze"); setLastName("Okafor"); }} />
          </div>
        </div>
            ...
            <div className="form-grid">
              <div className="field-wrap full">
                ...
                <div className="password-wrap">
                  <input className={`field-input${pwErrors.cur ? " error" : ""}`}
                    type={showPw.cur ? "text" : "password"} placeholder="Enter current password"
                    value={currentPw} onChange={e => { setCurrentPw(e.target.value); setPwErrors(x => ({ ...x, cur: "" })); }} />
                  <button className="password-toggle" onClick={() => setShowPw(s => ({ ...s, cur: !s.cur }))}>
                    {showPw.cur ? "Hide" : "Show"}
                  </button>
                </div>
                {pwErrors.cur && <div className="field-error">{pwErrors.cur}</div>}
              </div>

              <div className="field-wrap">
                <label className="field-label">New password</label>
                <div className="password-wrap">
                  <input className={`field-input${pwErrors.new ? " error" : ""}`}
                    type={showPw.new ? "text" : "password"} placeholder="Min. 8 characters"
                    value={newPw} onChange={e => { setNewPw(e.target.value); setPwErrors(x => ({ ...x, new: "" })); }} />
                  <button className="password-toggle" onClick={() => setShowPw(s => ({ ...s, new: !s.new }))}>
                  ...
                  <div className="password-strength">
                    {[1,2,3,4].map(i => (
                      <div key={i} className="strength-bar"
                        style={{ background: i <= strength ? STRENGTH_COLORS[strength] : "var(--cream-dark)" }} />
                    ))}
                    <span style={{ fontSize: "0.7rem", fontWeight: 700, color: STRENGTH_COLORS[strength], marginLeft: 4 }}>
                      {STRENGTH_LABELS[strength]}
                    </span>
                  </div>
                )}
                {pwErrors.new && <div className="field-error">{pwErrors.new}</div>}
              </div>

              <div className="field-wrap">
                <label className="field-label">Confirm new password</label>
                <div className="password-wrap">
                  <input className={`field-input${pwErrors.con ? " error" : ""}`}
                    type={showPw.con ? "text" : "password"} placeholder="Re-enter new password"
                    value={confirmPw} onChange={e => { setConfirmPw(e.target.value); setPwErrors(x => ({ ...x, con: "" })); }} />
                  <button className="password-toggle" onClick={() => setShowPw(s => ({ ...s, con: !s.con }))}>
                    {showPw.con ? "Hide" : "Show"}
                  </button>
                </div>
                {pwErrors.con && <div className="field-error">{pwErrors.con}</div>}
              </div>
            </div>

            <SaveRow loading={pwLoading} saved={pwSaved} onSave={savePassword}
              onReset={() => { setCurrentPw(""); setNewPw(""); setConfirmPw(""); setPwErrors({}); }} />
          </div>
        </div>
        ...
        <div className="settings-card" style={{ "--delay": "0.1s" }}>
          <div className="card-header">
            <div>
              ...
              <div className="card-sub">Used across all budgets, expenses, and AI insights</div>
            </div>
          </div>
          <div className="card-body">
            <div className="currency-grid">
              {CURRENCIES.map(c => (
                <div key={c.code} className={`currency-tile${currency === c.code ? " active" : ""}`}
                  onClick={() => setCurrency(c.code)}>
                  {currency === c.code && <div className="currency-check">✓</div>}
            ...
            <SaveRow loading={currLoading} saved={currSaved} onSave={saveCurrency}
              onReset={() => setCurrency("NGN")} />
          </div>
        </div>
        ...
        <div className="settings-card" style={{ "--delay": "0.13s" }}>
          <div className="card-header">
            <div>
              ...
              <div className="card-sub">Control what Truvllo sends you and when</div>
            </div>
          </div>
          <div className="card-body">
            ...
            <SaveRow loading={notifLoading} saved={notifSaved} onSave={saveNotifs}
              onReset={() => setNotifs({ daily_summary:true, over_pace_alert:true, trial_reminder:true, weekly_report:false, tips_and_advice:true })} />
          </div>
        </div>
        ...
        <div className="settings-card" style={{ "--delay": "0.16s" }}>
            ...
            {isTrialing ? (
              <>
                <div className="plan-hero">
                  ...
                  <div className="plan-badge"><span className="plan-badge-dot" />Premium Trial Active</div>
                  <div className="plan-name">7-Day Free Trial</div>
                  <div className="plan-desc">You're enjoying full Premium access. Upgrade before your trial ends to keep all features.</div>
                  <div className="plan-trial-bar">
                    ...
                    <div className="plan-bar-track">
                      <div className="plan-bar-fill" style={{ width: `${(trialDaysLeft / 7) * 100}%` }} />
                    </div>
                  </div>
                </div>
                ...
                <div className="plan-features">
                  {PLAN_FEATURES_PREMIUM.map(f => (
                    <div key={f} className="plan-feature-row"><span className="check">✓</span>{f}</div>
                  ))}
                </div>

                <button className="plan-upgrade-btn">Upgrade to Premium — ₦6,500/mo</button>
                <div style={{ textAlign:"center", fontSize:"0.75rem", color:"var(--ink-subtle)", marginTop:10 }}>
              ...
              <>
                <div className="plan-hero">
                  <div className="plan-hero-blob" />
                  <div className="plan-badge"><span className="plan-badge-dot" />Active</div>
                  <div className="plan-name">Premium Plan</div>
                  <div className="plan-desc">Next billing: April 16, 2026 · ₦6,500</div>
                </div>
                <div className="plan-features">
                  {PLAN_FEATURES_PREMIUM.map(f => (
                    <div key={f} className="plan-feature-row"><span className="check">✓</span>{f}</div>
                ...
                <button className="plan-renew-btn">Manage subscription</button>
              </>
            ) : (
              <>
                <div style={{ marginBottom: 16 }}>
                  <div style={{ display:"flex", alignItems:"center", gap:12, marginBottom:16 }}>
                    <div style={{ width:48, height:48, borderRadius:13, background:"var(--cream-dark)", display:"flex", alignItems:"center", justifyContent:"center", fontSize:"1.3rem" }}>🎯</div>
                    <div>
                      <div style={{ fontWeight:700, fontSize:"0.95rem" }}>Free Plan</div>
                      <div style={{ fontSize:"0.78rem", color:"var(--ink-subtle)", marginTop:2 }}>Core features included · AI features locked</div>
                    </div>
                  </div>
                  <div className="plan-features">
                    ...
                    {PLAN_FEATURES_PREMIUM.map(f => (
                      <div key={f} className="plan-feature-row" style={{ opacity:0.45 }}><span className="lock">🔒</span>{f}</div>
                    ))}
                  </div>
                </div>
                <button className="plan-upgrade-btn">Upgrade to Premium — ₦6,500/mo</button>
          ...
          <div className="danger-section">
            <div>
              ...
              <div className="danger-sub">Sign out of Truvllo on this device</div>
            </div>
            <button className="danger-btn" onClick={() => setConfirmModal("logout")}>Sign out</button>
          </div>
          <div className="card-divider" />
          <div className="danger-section">
            <div>
              <div className="danger-title">Delete account</div>
              <div className="danger-sub">Permanently delete your account and all data. This cannot be undone.</div>
            </div>
            <button className="danger-btn" onClick={() => setConfirmModal("delete")}>Delete account</button>
          </div>
        </div>
